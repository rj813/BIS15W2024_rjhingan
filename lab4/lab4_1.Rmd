...
```
# Select columns that meet specific criteria
...
```{r}
# handy bit of code for the project 
...
```{r}
#to select all the numeric data in the data set
select_if(fish, is.numeric)
```

To select all columns that are *not* a class of data, you need to add a `~`.
```{r}
#to select data that is not a certain class of data, the "!" means NOT
select_if(fish, ~!is.numeric(.))
```
...
```{r}
# There are no NAs, but there may be errors in the data set
anyNA(mammals)
```

```{r}
max_life_avg <- mean(mammals$`max. life`)
```
...
```{r}
# The names of the columns are order, family, Genus, species, mass, gestation, newborn, weaning, wean mass, AFR, max. life, litter size, litters/years
colnames(mammals)
```
...
```{r}
mass <- data.frame(select(mammals_new, "genus", "species", "mass"))
```
