...
```{r}
# NOTICE there are 2 equal signs and the value is in quotes
...
```{r}
# filter the fish within(%in%) the variable length 167 and 175
filter(fish, length %in% c(167, 175))
```

Alternatively, you can use `between` if you are looking for a range of specific values.  
```{r}
# between can give a RANGE of values
...
```{r}
# look for radi length near 2 within 0.2
