...
deserts %>%
  filter(species_id=="DM" | species_id=="DS") %>%
  mutate(year=as.factor(year)) %>% # makes the year a factor so the years each comes up individually on our graph
  group_by(year, species_id) %>%
  summarize(n=n(), .groups = 'keep') %>%
  ggplot(aes(x=year, y=n, group = species_id, color=species_id)) +
  geom_line() +
  geom_point(shape=13) + # you can experiment with different shapes
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_histogram(color = "black", fill = "gray85", bins=10)+
  labs(title = "Distribution of Body Mass")
```
...
homerange %>%
  ...
  geom_histogram(fill="yellow", color = "black", alpha = 0.5) +
...
homerange %>%
  ggplot(aes(x=log10.hra)) +
  geom_histogram(aes(y=after_stat(density)), fill="violetred4", alpha=0.3) +
