...
+`scale_colour_brewer()` is for coloring points  
+`scale_fill_brewer()` is for coloring fills  
...
```{r}
super_palette <- paletteer_d("palettetown::teamrocket")
```

```{r}
q + scale_fill_manual(values=super_palette)
```
...
homerange %>% 
  ggplot(aes(x=taxon, y=log10.mass))+
  geom_boxplot()+
  facet_grid(trophic.guild~.) + #facet_grid allows you to shape the plot by a categorical variable
  theme(axis.text.x = element_text(angle = 60, hjust=1))
```
...
homerange %>% 
  ggplot(aes(x=taxon, y=log10.mass))+
  geom_boxplot()+
  facet_wrap(trophic.guild~thermoregulation) +
  theme(axis.text.x = element_text(angle = 60, hjust=1))
```
