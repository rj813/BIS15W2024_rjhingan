...
```{r}
# The data is not tidy and it is in long format
```
...
```{r}
new_sydneybeaches_long <- sydneybeaches_long %>%
  separate(date, into=c("month", "day", "year"), sep="/") %>%
  group_by(site, year) %>%
  summarise(mean_beach = mean(enterococci_cfu_100ml, na.rm = T))
...
# Little Bay Beach
new_sydneybeaches_long %>%
  select(year, mean_beach) %>%
  filter(year == 2013) %>%
  arrange(desc(mean_beach))
```
