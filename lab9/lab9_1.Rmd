...
# Not tidy because drugs a,b,c,d are being treated as values instead of observations
(1) each variable has its own column?  
...
```{r}
# Every variable needs its own column in R!
relig_income %>%
...
```{r}
# The data is not tidy because the days are listed out in column names rather than having just one column for day
```
