...
```{r}
heartrate2 %>% 
  separate(patient, into= c("patient", "sex"), sep = "_") # separating variables that contain two observations into two separate variables
```
...
```{r}
heartrate3 %>% 
  unite(patient_sex, "patient", "sex", sep=" ") #combines two observations into one variable
```
...
# take values and make them separate variables
```
