...
```{r}
wolves$park <- as_factor(wolves$park)
levels(wolves$park)
```
...
wolves %>%
  group_by(park) %>%
  summarise(large_pack=sum(packsize_aug, na.rm = T)) %>%
...
wolves %>%
  group_by(park) %>%
  summarise(large_hkill=sum(mort_all, na.rm = T)) %>%
  arrange(desc(large_hkill))
```
...
ynp %>%
  filter(pack == "druid") %>%
  summarise(druid_mean=mean(packsize_aug, na.rm = T))
```
...
```{r}
#There may have been a major natural disaster in 2010 or no observations were made.
ynp %>%
  filter(pack == "druid") %>%
  group_by(biolyr) %>%
...
```{r}
#Among the YNP packs, mollies have the highest overall persistence and specialize in hunting bison.
ynp %>%
  group_by(pack) %>%
  summarise(high_persistence=sum(persisty1, na.rm = T)) %>%
  arrange(desc(high_persistence))
```
...
```{r}
#In this data set form 2010 to 2020, the "junction" pack recorded the largest pack size at 35.0 in 2020 at Yellowstone National Park.
wolves %>%
  filter (biolyr >= 2010 & biolyr <= 2020) %>%
  group_by(pack) %>%
  arrange(desc(packsize_aug))
```
