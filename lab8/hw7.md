...
```r
# The NAs are represented by "-999"
...
```r
amniota = mutate_all(amniota,~replace(., . == -999, NA))
amniota
```
...
```r
# Yes this makes sense. Mammals do not lay eggs and there is 100% missing data there. Aves and Reptilia both lay eggs and data existed for both.
amniota %>%
  group_by(class) %>%
...
```r
# The NAs are meant to represent that an observation is not a part of that species classification.
amphibio %>%
...
```r
amniota <- read_csv("data/amniota.csv", na= c("NA", " ", ".", "-999")) %>% clean_names()
```
