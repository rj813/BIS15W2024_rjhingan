...
```{r}
life_history %>%  #shows where and how many NAs there are
  map_df(~ sum(is.na(.)))
```
...
```{r}
life_history <- read_csv("data/mammal_lifehistories_v3.csv", na= c("NA"," ", ".", "-999")) %>% clean_names() #replace the bad values with NA, clean the data 
...
```{r}
naniar::miss_var_summary(life_history) # Shows how many missing observations there are and what percent of the data is missing
```
...
```{r}
vis_miss(life_history) #shows where missing data lies in the data set
```
